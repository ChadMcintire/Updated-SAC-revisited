...
class ReplayBuffer:
    ...
    def __init__(
        ...
        self.save_entropy_flag: bool = save_entropy

        if self.save_entropy_flag:
            self._reserved_keys = ("obs", "act", "rew", "done", "obs_next", "info", "policy", "old_entropy")
        self.reset()
    ...
    def __getitem__(self, index: Union[slice, int, List[int], np.ndarray]) -> Batch:
        ...
        if not self.save_entropy_flag:
            ...
            return Batch(
                ...
                info=self.get(indices, "info", Batch()),
                policy=self.get(indices, "policy", Batch()),
                old_entropy = self.old_entropy[indices],
            )
